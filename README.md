...
## 🚀 最新リリース: FX 5分足スキャルピングEA

ボブ・ボルマン著「FX 5分足スキャルピング」の手法を実装した最新のEAです。

### 主な機能
- **パターンブレイク検出**: レンジブレイクアウトの自動検出
- **ビルドアップ分析**: 価格圧縮パターンの高精度認識
- **25EMAフィルター**: トレンド方向の確認
- **リスク管理**: 自動ロット計算、日次損失制限
- **情報パネル**: リアルタイム統計表示

詳細は [📚 FX 5分足スキャルピング ドキュメント](docs/fx-scalping/) を参照してください。

### クイックスタート
1. EUR/USD 5分足チャートを開く
2. `FX5MinScalpingEA.mq4` を適用
3. 自動売買をONにする

推奨設定は [テストガイド](docs/fx-scalping/TESTING_GUIDE.md) を参照してください。
